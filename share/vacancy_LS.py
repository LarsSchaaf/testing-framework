...
def do_one_vacancy(
    ...
    if relax_radial != 0.0 or relax_symm_break != 0.0:
        ...
        for i, offset in zip(indices, offsets):
            ri = vac.positions[vac_i] - (
                vac.positions[i] + np.dot(offset, vac.get_cell())
            )
...
def do_all_vacancies(fname, nn_cutoff=0.0, tol=1.0e-2, dummy=False):
    ...
    Only considers ineqivalent atom vacanceis

    Args:
        fname (str): file name of .xyz file
        nn_cutoff (float, optional): nearest neighbour cutoff. Defaults to 0.0.
        tol ([type], optional): relaxation tollerance. Defaults to 1.0e-2.

    Returns:
        dict: properites for .json file
    """
    print(f"All vacancies for: {fname}")
    bulk_supercell = ase.io.read(fname, format="extxyz")
    ...
    if True:  # bulk_supercell.info["vacancies"] == "inequivalent":
        ...
        if "arb_supercell" in bulk_supercell.info:
            ...
            for i in prim_vacancy_list:
                ...
                print(
                    "found closest in new cell",
                    i_closest,
                    "distance in orig cell lattice coords",
                    np.dot(
                        (bulk_supersupercell.get_positions()[i_closest] - p),
...
def vacancy_all_xyz(direct, nn_cutoff=0.0, tol=1.0e-2, dummy=False):
    ...
    Performes do_all_vacancies on all .xyz files in the directory thats of interest.

    Args:
        diret (str): directory to analyse
    """

    all_xyzs = os.path.join(direct, "*.xyz")

    fnames = glob(all_xyzs)
