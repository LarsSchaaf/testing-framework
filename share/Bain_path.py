...
def do_Bain_path(bulk_struct_test_fcc, bcc_fcc_steps=15, extra_deformation=0.3):
    ...
    at0_fcc = ase.atoms.Atoms(
        cell=lattice, scaled_positions=scaled_positions, numbers=numbers, pbc=[True] * 3
    )
    ...
    for c_a in np.linspace(c_a_max, c_a_min, n_steps):
        ...
        at = relax_config(
            ...
            keep_symmetry=True,
            config_label=f"Bain_path_{c_a:.3}",
            save_traj=True,
        ...
        energies.append(
            (c_a, at.get_potential_energy() / len(at), at.get_stress().tolist())
        )
