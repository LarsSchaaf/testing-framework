...
def sd2_run(
    ...
    converged,
    max_iter=1000,
    max_cell_vec_change_ratio=3.0,
    exception_if_invalid_config=None,
):
    traj = []
    ...
    for i_minim in range(max_iter):
        ...
        if done:
            print(
                log_prefix,
                "SD2: i {} E {} {} {}".format(
                    i_minim, E, config_minim.log_message, done
                ),
            )
            sys.stdout.flush()
        ...
        print(
            log_prefix,
            "SD2: i {} E {} {} alpha {} {}".format(
                i_minim, E, config_minim.log_message, alpha, done
            ),
        )
        sys.stdout.flush()
    ...
    if try_restart:
        ...
        if from_base_model:
            ...
            try:
                ...
                try:
                    print(
                        "relax_config failed to read base run config from ",
                        base_run_root + "-" + config_label + "-relaxed.xyz",
                    )
                except:
...
def evaluate_file(
    file, do_energy=True, do_forces=True, do_stress=True, do_predictive_error=None
):
    al = read(file, index=":")
    for a in al:
...
def evaluate(
    atoms, do_energy=True, do_forces=True, do_stress=True, do_predictive_error=None
):
    import model
    ...
    if do_predictive_error:
        try:
            ...
            if isinstance(do_predictive_error, float):
                ...
                raise ValueError(
                    "do_predictive_error not float or bool {}".format(
                        str(do_predictive_error)
                    )
                )
    ...
    spc = SinglePointCalculator(atoms, energy=energy, forces=forces, stress=stress)
    atoms.set_calculator(spc)

    if do_predictive_error:
        atoms.arrays["predictive_error"] = np.sqrt(
            model.calculator.results["predictive_error"]
        )
        try:
    ...
    while not done and i_iter < max_n_lbfgs:
        try:
            ...
            relax_config(
                ...
                method="lbfgs",
                keep_symmetry=keep_symmetry,
                config_label=f"{label}.{i_iter}",
            )
            done = atoms.info["n_minim_iter"] < max_lbfgs_iter
            print(
                "robust_minim relax_configs LBFGS finished in ",
                atoms.info["n_minim_iter"],
                "iters, max",
                max_lbfgs_iter,
            )
        except:
...
def get_relaxed_bulk(bulk_struct_test, model_name=None):
    bulk_model_test_relaxed = os.path.join(
        "..",
...
def rescale_to_relaxed_bulk(supercell):
    ...
    try:
        ...
        cell_ratio = np.linalg.norm(supercell_a1_in_bulk) / np.linalg.norm(
            supercell.get_cell()[0, :]
        )
        if "supercell_a2_in_lattice_coords" in supercell.info:
            raise ValueError(
                "anisotropic rescaling of supercellace cell not implemented"
            )
