...
parser.add_argument(
    ...
    type=str,
    action="store",
    help="label for tests directory",
    default="",
)
parser.add_argument(
    "--force", "-f", action="store_true", help="force rerunning of test"
)
parser.add_argument(
    "--base_model",
    "-B",
    type=str,
    action="store",
    help="optional base model to start from",
)
parser.add_argument(
    "--no_redirect_io", "-N", action="store_true", help="do not redirect io"
)
parser.add_argument(
    "--no_append_log",
    dest="append_log",
    action="store_false",
    help="overwrite log (.txt) file, rather than appending",
)
...
if (
    (
        "GAP_TESTER_CHECKPOINT" in os.environ
        and os.environ["GAP_TESTER_CHECKPOINT"] != ""
    )
    or not hasattr(model, "no_checkpoint")
    or not model.no_checkpoint
):
