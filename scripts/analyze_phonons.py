...
for model_name in models:
    ...
    for (bulk_i, bulk_struct_test) in enumerate(bulk_struct_tests):
        ...
        ax_BP = fig_BP.add_subplot(
            1, 1, 1
        )  # fig_BP.add_subplot(len(data[ref_model_name]["phonons"]),1,bulk_i+1)
        ...
        if "BAND_PATH" in model_data:
            ...
            f = open(
                "phonons_BAND_PATH-" + model_name + "-" + bulk_struct_test + ".dat", "w"
            )
            ...
            ylim = ax_BP.get_ylim()
            for p, l in zip(
                model_data["BAND_PATH"]["positions"][1:-1],
                model_data["BAND_PATH"]["labels"][1:-1],
            ):
                if l != ".":
...
fig_DOS.savefig("ls-phonons_DOS-" + model_name + ".pdf")
pyplot.clf()
