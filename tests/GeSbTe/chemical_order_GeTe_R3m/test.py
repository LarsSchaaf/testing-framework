...
for config_i in range(20):
    ...
    unrelaxed_energies.append(
        antisites.get_potential_energy() / len(antisites) - relaxed_bulk_pe
    )
    relaxed_antisites = antisites.copy()
    relaxed_antisites = relax_config(
        relaxed_antisites,
        relax_pos=True,
        relax_cell=True,
        tol=tol,
        traj_file=None,
        config_label="supercell_antisite_{}".format(config_i),
        from_base_model=True,
        save_config=True,
    )
    relaxed_energies.append(
        relaxed_antisites.get_potential_energy() / len(antisites) - relaxed_bulk_pe
    )
    ase.io.write(sys.stdout, relaxed_antisites, format="extxyz")

properties = {
    "unrelaxed_energy_per_atom": unrelaxed_energies,
