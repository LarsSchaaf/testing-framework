...
def struct_energy(e0_per_atom, filename):
    ...
    ats = ase.io.read(
        os.path.dirname(__file__) + "/" + filename, index=":", format="extxyz"
    )
    for (i, at_orig) in enumerate(ats):
        ...
        if os.path.basename(os.path.dirname(model.__file__)) == "CASTEP_file":
            ...
            for i_minim in range(500):
                ...
                if (
                    np.max(np.abs(grad_f[:n_at])) < fmax
                    and np.max(np.abs(grad_f[n_at:])) < fmax
                ):
                    converged = True
                    break
                if i_minim == 0:
                    ...
                    alpha = np.sum((x - x_old) * (grad_f - grad_f_old)) / np.sum(
                        (grad_f - grad_f_old) ** 2
                    )
                x_old = x.copy()
...
(V_unrel, E_unrel, V_rel, E_rel) = struct_energy(
    e0_per_atom, "model-GAP-6-test-RSS_like_open_ended-unique-minima.xyz"
)
properties = {
    "V_unrelaxed": V_unrel,
    "E_unrelaxed": E_unrel,
